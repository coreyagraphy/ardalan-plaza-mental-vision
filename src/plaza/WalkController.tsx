...
export function WalkController() {
  ...
  useFrame((_, dt) => {
    ...
    pos.current.x = Math.max(-70, Math.min(70, pos.current.x));
    pos.current.z = Math.max(-16.4, Math.min(55, pos.current.z));
    // Foyer cavity: enter only through the open glass doors, stay inside walls.
    const inTowerX = pos.current.x > -5.7 && pos.current.x < 5.9;
    const doorX = pos.current.x > -1.25 && pos.current.x < 1.35;
    if (pos.current.z < 0.12 && pos.current.z > -16.1) {
      if (!inTowerX) {
        if (pos.current.x <= -5.7) pos.current.x = -5.7;
        if (pos.current.x >= 5.9) pos.current.x = 5.9;
      }
      if (pos.current.z < 0.12 && pos.current.z > -0.05 && !doorX) {
        pos.current.z = 0.14;
      }
    }
