...
function SceneBody() {
  ...
  return (
    <>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[2, 0.018, 4]} receiveShadow>
...
function OrbitRig() {
  ...
  useEffect(() => {
    ...
    c.target.set(
      CAMERAS.establishing.target[0],
      CAMERAS.establishing.target[1],
      CAMERAS.establishing.target[2],
    );
    controls.current = c;
    return () => {
      c.dispose();
...
export function PlazaScene() {
  ...
  return (
    <>
      ...
      <CaptureBridge />
      <PostFX />
      {!film && !walk && camera === "orbit" && <OrbitRig />}
    </>
  );
}
