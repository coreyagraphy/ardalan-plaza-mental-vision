import { BUILDING, FOOTPRINT, FOURTH_AVE, FOUNTAIN, NORTH_Z, ROUNDABOUT, STREET, THIRD_AVE, TREES } from "./config";
...
function ClockArch({ mats }: { mats: PlazaMats }) {
  const x = ROUNDABOUT.x - 11.4;
  const z0 = 1.65;
  const z1 = 14.4;
  const mid = (z0 + z1) / 2;
  const span = z1 - z0;
  return (
    <group name="CLOCK_ARCH" position={[x, 0, 0]} userData={{ assumption: "district gateway, estimated" }}>
      {[z0, z1].map((z) => (
        <group key={z} position={[0, 0, z]}>
          <mesh position={[0, 3.1, 0]} material={mats.brickFacade} castShadow>
            <boxGeometry args={[2.15, 6.2, 2.15]} />
          </mesh>
          <mesh position={[0, 6.35, 0]} material={mats.limestone} castShadow>
            <boxGeometry args={[2.45, 0.35, 2.45]} />
          </mesh>
          <mesh position={[0, 7.55, 0]} material={mats.brickFacade} castShadow>
            <boxGeometry args={[1.55, 2.1, 1.55]} />
          </mesh>
          <mesh position={[0, 8.7, 0]} material={mats.limestone} castShadow>
            <boxGeometry args={[1.75, 0.28, 1.75]} />
          </mesh>
        </group>
      ))}
      <mesh position={[0, 6.55, mid]} material={mats.iron} castShadow>
        <boxGeometry args={[0.45, 0.55, span - 1.6]} />
      </mesh>
      <mesh position={[0, 7.15, mid]} material={mats.iron} castShadow>
        <boxGeometry args={[0.18, 0.7, span - 2.2]} />
      </mesh>
      {[-2.2, 0, 2.2].map((dz) => (
        <mesh key={dz} position={[0, 6.85, mid + dz]} material={mats.iron}>
...
export function StreetContext({ mats, dusk }: { mats: PlazaMats; dusk: boolean }) {
  ...
  return (
    <group name="05_MAIN_STREET">
      ...
      {/* Chamfer sidewalk wrapping the corner into the roundabout */}
      <mesh
        rotation={[-Math.PI / 2, 0, 0]}
        position={[BUILDING.west + 1.2, 0.034, BUILDING.north - 1.2]}
        material={mats.brick}
        receiveShadow
      >
      ...
      <group name="07_LANDSCAPE">
        ...
        <mesh position={[FOOTPRINT.west - 1.5, 0.38, -18.2]} material={mats.hedge} castShadow>
          <boxGeometry args={[0.7, 0.7, 8.4]} />
        </mesh>
      </group>
